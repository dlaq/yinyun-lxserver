...
补齐队列会在打开“曲库联动”页面时加载，并在页面处于前台时每 2.5 秒自动刷新；点击“一键补齐全部缺失”成功入队后会立即重新读取队列和导入匹配结果。失败行会保留错误原因，并提供“重试”和“移除”两个动作：先按错误详情处理（例如音源 HTTP 404 时刷新音源或稍后重试），确认当前音源始终没有可下载地址后再移除任务。移除只删除队列中的待处理记录，不删除已经存在的音乐文件。
