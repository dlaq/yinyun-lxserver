...
历史实现曾允许播放器自动发出 `push + replace`，也会把“空音云快照”解释成“期望远端集合为空”。前者会删除 Songloft 中仅远端存在的歌曲，后者会把目标清成 0 首；这两条都是破坏性路径。当前实现遵守以下规则：

- 播放器自动同步和普通用户手动同步固定使用 `push + merge`，只能追加，不能删除 Songloft 现有歌曲。
- 覆盖仅允许管理员使用，并且必须明确选择 Songloft 目标；空源、全未匹配、重复的远端歌曲 ID 和两个音云歌单映射到同一个 Songloft 目标都会被拒绝，不存在绕过空源保护的参数。
- 管理员覆盖先用 `dryRun: true` 获取当前/期望/新增/移除数量和一次性确认令牌；实际覆盖必须在五分钟内回传该令牌。令牌使用一次即销毁，源或目标状态变化后也会失效。
- 覆盖前连续读取两次 Songloft 目标，声明数量、实际数量和两次快照必须一致；不稳定或结构异常的上游响应会终止同步。
- 写入前将完整原始歌曲顺序和源/目标哈希原子保存到 `playlist-replace-backups/<user>/<backupId>.json`，再先添加后移除；写入后按歌曲 ID 和顺序精确核验。
- 任一步骤失败都会按持久化备份恢复原有歌曲集合和顺序，并再次精确核验；若回滚也失败，接口返回备份 ID，供管理员人工恢复和审计。
- 删除音云歌单只删除本地歌单和同步映射，不再隐式删除 Songloft 歌单。远端删除必须调用明确的 Songloft 删除动作。

生产账本证明白噪音歌单在 2026-08-25 的同步结束时，音云源仍有 61 首，而 Songloft 快照已为 0 首；同一时间附近存在覆盖/合并请求，但现存访问日志没有保留请求体，因此不能严谨断言历史那一次具体由哪条请求清空。上述保护关闭了本项目已确认的全部直接清空路径；若再次出现 0 首，还应结合持久化覆盖审计、Songloft 服务日志和同步账本时间戳核对外部客户端写入、扫描异常或远端 API 的非原子行为。
...
同步账本保存在音云数据目录 `playlist-sync/<user>.json`，记录双方最后共同歌曲集合及哈希，用于后续合并和冲突提示。默认 `merge` 只追加，不删除远端歌曲；只有管理员明确使用 `mode: "replace"` 的 `push`，并完成预演与二次确认后，才会移除 Songloft 歌单中的多余歌曲。覆盖审计和操作前快照另外保存在 `playlist-replace-backups/<user>/`，不会被同步账本覆盖。
...
选择目标后，`merge` 只追加不删除；只有管理员明确选择 `push + replace`、预演并二次确认，才会按音云歌单覆盖 Songloft。覆盖同步在存在未匹配歌曲、空源、目标读数不稳定或确认令牌过期时会整体取消，不会先删除远端已有歌曲。
