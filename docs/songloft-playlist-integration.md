...
- Songloft 的系统歌单（当前内置 ID `1`、`2`、`built_in` 标签或 `radio` 类型）只读，不能被选作同步目标。
- 同一个 Songloft 歌单只能由一个音云歌单映射；发现历史账本中的重复映射时返回 `409 songloft_playlist_already_mapped`，不会先重命名或写入远端。
- 如果远端读取突然返回空集合，但同步账本记录此前存在共同歌曲，本次覆盖会跳过重排/删除操作，只允许安全的新增；下一次读取确认远端成员后再继续覆盖。
- 同一 Songloft 目标的并发同步会返回 `409 songloft_playlist_sync_in_progress`，避免两个请求交叉执行添加、删除和重排。
