...
9. Songloft 覆盖同步必须保留空源拒绝、先添加后删除、写后校验/失败恢复、异常空快照跳过重排、单目标锁和一对一映射保护。
系统/电台歌单只读；删除音云歌单只能移除本地映射，不能顺带删除远端 Songloft 歌单。
...
6. Songloft 返回空集合但账本显示目标此前有歌曲时，只允许安全新增并跳过覆盖重排；同一远端歌单同时只允许一个同步请求，且一个远端 ID 不能映射到其它音云歌单。
