...
- **多用户复制与共享本地曲库。** 每个账户的自定义音源、平台开关和歌单仍独立保存；管理员可在 `/admin` 的“跨用户同步”中，把指定用户的音源批量追加/覆盖到一个或多个用户，或把一个用户的歌单追加/覆盖到另一个用户。所有已登录用户都能只读浏览、播放并收藏其他用户 `/music/<用户名>` 中的本地音乐；临时 `/cache`、删除、重命名和元数据修改仍严格属于文件所有者。
...
> [!WARNING]
> Songloft “覆盖”同步是显式的破坏性操作：仅管理员可以执行；源歌单为空时默认拒绝；写入顺序为先添加、后删除，并在写后校验失败时尽力恢复同步前的 Songloft 歌曲。普通用户和播放器自动同步固定使用 `push + merge`，删除音云歌单也不会隐式删除 Songloft 歌单。
