...
### 架构治理与生产安全（2026-09-05）

- 合并上游更新，并保留本项目的移动端长列表、确定性返回、PWA 隔离、跨用户同步和 Songloft 安全边界。
- 管理员跨用户歌单/音源同步统一为“预览、一次性确认、操作日志、全目标回滚”，移除旧的非事务直写入口。
- 登录凭据迁移为 scrypt；兼容密码由 AES-256-GCM 加密，访问/刷新会话支持轮换、撤销与重启持久化。
- 关键 JSON 增加 schema、revision、原子写入、有效备份、损坏证据和失败关闭；增加历史重复歌单专用修复流程。
- Songloft 替换同步拒绝空源、异常响应、过期预览和写后数量不符；自动同步和普通用户操作保持追加语义。
- 修复歌单详情返回后页面闪烁、空白和无法再次进入；后台网络歌单状态只局部更新卡片。
- 播放器、歌单、排行榜、本地音乐和曲库联动采用页面级长滚动，并覆盖 iPhone/Android 常见宽度和安全区域。
- PWA 预缓存清单自动生成，播放器与管理端缓存按构建隔离；导航与配置 network-first，音频、API、动态封面和敏感响应不缓存。
- 远程媒体代理增加逐跳 DNS/重定向校验、IPv4/IPv6 私网与保留地址拦截、DNS pin、超时和响应大小限制。
- 删除用户时同步终止下载/洗版任务，并清理凭据、会话、分享关系、音源偏好、联动账本、外部索引和专属缓存。
- 新增 `docs/architecture.md`，记录模块职责、数据流、依赖、设计模式、架构风险和发布门槛。
